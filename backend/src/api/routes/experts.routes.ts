...
router.put('/me', authenticateJWT, (req: AuthenticatedRequest, res: Response) => {
  const expert = Object.values(mockExperts).find((e) => e.userId === req.user?.id) || mockExperts['exp_marcus_1'];
...
router.post('/verification', authenticateJWT, (req: AuthenticatedRequest, res: Response) => {
...
router.get('/verification', authenticateJWT, (req: AuthenticatedRequest, res: Response) => {
  const expert = Object.values(mockExperts).find((e) => e.userId === req.user?.id) || mockExperts['exp_marcus_1'];
