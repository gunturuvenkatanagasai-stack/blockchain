...
const mockDigitalHumans: Record<string, any> = {
  'dt_marcus_algo': {
    id: 'dt_marcus_algo',
...
router.get('/:id', (req, res) => {
  ...
  if (!dh) return res.status(404).json({ error: 'Digital Human not found' });
  return res.json({ digitalHuman: dh, digitalTwin: dh });
});

// POST /api/v1/digital-humans
router.post(['/', '/digital-twins'], authenticateJWT, (req: AuthenticatedRequest, res: Response) => {
...
router.post('/:id/publish', authenticateJWT, (req: AuthenticatedRequest, res: Response) => {
  const dh = mockDigitalHumans[req.params.id];
  if (!dh) return res.status(404).json({ error: 'Digital Human not found' });
  dh.status = 'PUBLISHED';
...
router.post('/:id/unpublish', authenticateJWT, (req: AuthenticatedRequest, res: Response) => {
  const dh = mockDigitalHumans[req.params.id];
  if (!dh) return res.status(404).json({ error: 'Digital Human not found' });
  dh.status = 'DRAFT';
