...
import { x402PaymentMiddleware, X402Request } from '../../x402/x402.middleware';
 ...
 */
function generateIntelligentResponse(digitalTwinId: string, query: string, mode: string): { content: string; citations: any[] } {
...
async function handleChatInference(req: X402Request, res: Response) {
  ...
  try {
    const aiRes = await axios.post(`${config.aiServiceUrl}/api/v1/inference`, {
...
router.post('/chat', x402PaymentMiddleware('id'), handleChatInference);
router.post('/chat/message', x402PaymentMiddleware('id'), handleChatInference);
router.post('/chat/query', x402PaymentMiddleware('id'), handleChatInference);
router.post('/digital-humans/:id/chat', x402PaymentMiddleware('id'), handleChatInference);
